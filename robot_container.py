...
class RobotContainer:
    ...
    def _setup_controller_bindings(self) -> None:
        ...
        commands2.button.Trigger(lambda: self._driver_controller.getRightTriggerAxis() > 0.75).whileTrue(
            self.drivetrain.apply_request(
        ...
        self._driver_controller.leftBumper().whileTrue(
            self.drivetrain.apply_request(lambda: DriverAssist()
                                          ...
                                          .with_translation_pid(Constants.AutoAlignConstants.TRANSLATION_PID)
                                          .with_heading_pid(Constants.AutoAlignConstants.HEADING_PID[0], Constants.AutoAlignConstants.HEADING_PID[1], Constants.AutoAlignConstants.HEADING_PID[2])
        ...
        self._driver_controller.rightBumper().whileTrue(
            self.drivetrain.apply_request(lambda: DriverAssist()
                                          ...
                                          .with_max_angular_rate(self._max_angular_rate)
                                          .with_translation_pid(Constants.AutoAlignConstants.TRANSLATION_PID)
                                          .with_heading_pid(Constants.AutoAlignConstants.HEADING_PID)
                                          .with_velocity_deadband(Constants.AutoAlignConstants.VELOCITY_DEADBAND)
                                          )
        )
